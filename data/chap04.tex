...
要实现区分化服务，首先需要使硬件能够识别出不同的应用，
并能够获得不同应用的服务质量需求信息。
传统的服务器并不能提供这样的功能，即使服务器上运行不同的应用，
硬件却不能对这些应用进行区分，只能进行无差别的处理，
无法满足应用不同的服务质量需求。
造成这一现状的原因是当前计算机体系结构在设计之初并没有考虑多租户使用场景，
硬件部件与数据通路中没有提供基于应用的区分和隔离功能，
要解决这一问题，需要在体系结构内提供一种软硬件的接口，
...
将应用的需求信息传递到硬件，
同时硬件也不再需要通过猜测的方式区分应用，
可以根据标签区分来自不同应用的请求并进行区分处理。
本章内容安排如下：
首先介绍现有软硬件层次的应用区分工作，并分析其不足；
之后介绍本文所提出的标签化地址空间的原理、实现难点、以及技术优势； 
最后在模拟器平台上实现了标签化地址空间，
...
实现多应用的区分化服务，其本质是通过硬件区分，实现应用之间的资源隔离与性能隔离。
%本节主要介绍现有计算机如何实现资源隔离与性能隔离，但是为啥没有提供区分化服务

\subsection{资源隔离}

从多进程技术出现开始，计算机就开始支持资源隔离，内存与I/O是主要的两类需要隔离的资源。
地址空间隔离是实现内存资源隔离的主要方式；
而对于I/O资源，目前普遍在软件层次（如操作系统或Hypervisor/VMM层）实现隔离。
本节将对这两种资源的隔离进行讨论。

\textbf{内存资源隔离}\quad
不同的进程之间具有独立的地址空间，
现代处理器通过内存管理单元（Memory Management Unit，MMU）实现进程间地址空间的隔离。
处理器核使用虚拟地址进行访问，MMU通过分页或分段机制将虚拟地址转换为物理地址，
操作系统分配并管理MMU使用的地址映射，
将不同进程映射的地址空间映射到不同的物理地址空间，实现地址空间的隔离。
以Intel的IA-32/64架构处理器为例，大多数操作系统仅使用其提供的分页机制，
由操作系统维护地址映射使用的页表，以进程为粒度实现地址空间隔离，
并通过处理器的CR3寄存器将页表地址传递给硬件MMU，通过地址映射实现内存资源的隔离。
%% !! 这里说这个有点太早
%由于处理器核被多个进程分时共享，而MMU内部的TLB使用虚拟地址做索引，
%因此为了防止不同进程相同地址的访问在TLB查找时发生冲突，
%在进程切换时会无效掉所有的TLB表项。
通过地址空间隔离可以很容易的实现内存资源以及Memory-Mapped I/O资源的隔离，
由操作系统管理系统内所有的内存与I/O资源，并负责为不同的进程分配资源，
实现资源隔离。

虚拟化技术出现后，计算机仍然使用这种基于地址空间隔离的方式实现资源隔离。
同样以Intel为例，其使用扩展页表（Extended Page Table, EPT）机制
在原有的分页机制基础上增加了一级额外的地址映射，用于支持虚拟化场景下的地址映射，即
线性地址（Linear Address）$\Rightarrow$
客户机物理地址（guest-physical address）$\Rightarrow$
主机物理地址（host-physical address）
三级地址映射。
此时地址空间隔离的粒度从进程扩展到虚拟机粒度，

\textbf{I/O资源隔离}\quad
对于I/O资源，操作系统或VMM负责管理整个系统中的I/O设备，
以linux操作系统为例，通过文件的形式为应用提供I/O设备的访问，
而VMM则通过虚拟设备向虚拟机提供I/O设备的访问。
这两种方式都是在软件层次实现I/O资源的隔离，
...
基于地址空间隔离能够实现很好的资源隔离，但在性能隔离方面则会变得无能为力。
由于是通过将不同应用映射到不同的资源的方式实现资源隔离，
硬件并不能区分出不同的应用，
针对多处理器芯片中多应用共享的特征，Intel提出了Resource Director Technology（RDT）
方案来解决多应用资源共享冲突问题。
...
现有技术能够通过地址空间隔离的方式实现资源隔离，
这种方式本质是使用物理地址区分不同的应用的访问；
但经过地址转换后，请求也就丢失了应用的信息，也就无法实现性能隔离。
Intel提出的RDT技术对这一缺陷进行补充，通过RMID/COS标签实现处理器末级缓存上的性能隔离；
但这些标签信息并没有传递到处理器末级缓存以下的硬件，使得只能在局部实现性能隔离。
如果能够将标签信息传递到整个计算机系统，那么就能够实现全局的性能隔离，
这也是本章所提出的标签化地址空间的核心。
...
综上，目前服务器实现资源隔离，都是以使用处理器为中心，通过地址映射方式实现资源隔离，
在硬件层次并不能区分出不同的应用，这造成了硬件无法识别出来自不同应用的请求，
也就无法实现性能隔离。
虽然Intel的RDT技术使用RMID/COS标签将应用信息传递到处理器末级缓存，
并在该层次实现了区分化服务与性能隔离，
但这些应用信息并没有继续传递到处理器末级缓存以下的硬件中，
使得其它硬件无法支持区分化服务。

对比以上两种隔离方式，标签是更好的隔离手段，完全可以统一使用标签实现资源隔离与性能隔离，
