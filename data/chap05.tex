...
基于以上分析，控制平面使用控制表作为数据平面的接口，其中包括三个功能不同的控制表，
分别是：
参数表（Parameter Table），用于记录数据平面机制所需的参数，用户通过修改参数表实现对数据平面的控制；
状态表（Statistics Table），用于记录数据平面的状态信息，具体记录什么样的状态信息由硬件资源及其数据平面决定；

% 表的具体结构，使用前一章提供的DS-id做索引，由数据平面对其列进行解析
% 请求到达后获取DS-id，查询参数表，获取参数，与请求一同传递到数据平面，并进行处理
% 请求处理完成后，将更新的状态信息与DS-id送回到控制平面，并更新状态表
...
% 控制平面提供接口实现对所有控制表的访问，具体参见下文
% 简单来说就是提供表访问的寄存器窗口，PRM通过控制平面网络实现访问


% 上面是poll的查询方式，需要大量的计算资源才能实现实时的状态更新
% 提供了一个触发表，用于快速响应，其中存储了<DSid, stat-id, 临界值>，基于该触发表可以实现实时的资源调整机制，参见下节
触发表（Trigger Table），记录状态触发条件与触发事件通知。


\subsection{资源调整机制}

% 基于上节的触发表，实现了``trigger$\Rightarrow$action''编程方法，用于实现资源调整
除了上节所描述的基本资源监控与管理机制，
