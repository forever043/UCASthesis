...
其内部结构如图\ref{fig:pard-processor}所示，
其中包含MicroBlaze软核、中断控制器和时钟模块，
该模块从外部输入时钟、复位和中断信号，
通过三个AXI接口（IC指令缓存端口、DC数据缓存端口和DP外设端口）与外部交换数据。
AXI总线协议中使用独立的通道实现读写请求、数据与响应，
...
用于将标签寄存器的值附加到AXI总线AR/AW两个通道的USER信号中，完成请求标记。
...
% 原型的照片
本文使用Xilinx Virtex-7 FPGA（型号xc7vx690tffg1761-2）实现了上述PARD原型系统，
如图\ref{fig:pard-photo}所示。
