...
\begin{figure}[tb]
  ...
  \caption{不同虚拟化场景下CPU性能对比\cite{Reddy:2014}}
...
可以看到使用虚拟化技术虽然节省了基础设施的开销，但却带来了额外的软件费用与性能下降，
使得单位负载开销反而不如在物理机上直接部署相应的web应用，
这说明当前虚拟化技术并不是非常完善的资源隔离技术，仍有改进空间。
...
Intel提供的是以路（way）为粒度的容量划分机制，大部在Cache上的工作也都是基于路划分的。%\cite{}。
