...
图\ref{fig:apss-corun-measuring}中java与其中5个应用混合运行时，性能也没有出现明显的变化。
...
\begin{minipage}{0.48\textwidth}
  ...
  \caption{不同应用对websearch应用QoS的影响\cite{mars_bubble-up:_2011}}
  \label{fig:apps-corun-bubbleup}
\end{minipage}\hfill
\begin{minipage}{0.48\textwidth}
  \centering
  \includegraphics[width=\textwidth]{bg/apps-corun-measuring}
  \caption{数据中心应用混合运行后干扰情况\cite{kambadur_measuring_2012}}
...
然后再与其它应用混合运行得到其性能下降，以些来获得应用之间的干扰情况。
...
1）评估应用对内存子系统产生的压力，以pressure参数表示；
2）评估不同压力下应用的性能下降程度，以sensitivity曲线表示。
通过以上2组参数，即可量化的描述应用与内存子系统之间的关系。
通过1个应用的pressure参数查询另1个应用的sensitivity曲线，
即可精确的得出2个应用混合运行后的性能干扰情况，以此为指导实现QoS-aware的任务调度策略。
